...
class Note:
    def __init__(self, date_time, text, sticky=False, uid=None,
        ...
        self.reminder_days = reminder_days  # for nonrepeating daily reminders
        self.last_repeat_reminder_date = last_repeat_reminder_date  # for nonrepeating daily reminders
        # New repeat fields:
        ...
        # New: option to stop frequent (dynamic) reminders
...
# -------------------------------
# MainWindow with Repeating/Reminding & Option to Stop Frequent Reminders
# -------------------------------
class MainWindow(QMainWindow):
    ...
    # ---------------------------
    # Dynamic Reminder on Occurrence Day
    # ---------------------------
    ...
    def compute_next_reminder(self, note):
        ...
        if now < target:
            # For repeating notes on occurrence day, check if suppression is enabled.
    ...
    def check_reminders(self):
        ...
        for note in self.notes:
            # For repeating notes: if current occurrence has passed, update it.
            ...
            if nxt and now >= nxt:
                if rtype == "pre-final" and not note.pre_final_triggered:
                    ...
                    if note.repeat_mode != "None" and now.date() == note.next_occurrence.date():
                        # If user has chosen to suppress dynamic reminders, skip dynamic reminders.
                        if note.suppress_dynamic_reminders:
                            # Wait until pre-final time.
                            continue
                        dyn = self.get_dynamic_reminder(note)
                        if dyn and now >= dyn:
    ...
    # ---------------------------
    # Toggle Sorting and Archive (with icon changes)
    # ---------------------------
    ...
    # ---------------------------
    # Repeat Mode UI Setup (includes new "Only Final Reminder" checkbox)
    # ---------------------------
    def setup_repeat_ui(self, parent_layout):
        ...
        # For Custom mode: spin boxes for days and hours.
        ...
        # New: Checkbox to stop frequent (dynamic) reminders.
        self.only_final_checkbox = QCheckBox("Only Final Reminder")
        self.only_final_checkbox.setToolTip("When checked, dynamic frequent reminders are suppressed for the occurrence day")
        self.only_final_checkbox.setVisible(False)
        self.only_final_checkbox.toggled.connect(self.on_only_final_changed)
        parent_layout.addWidget(self.only_final_checkbox)
    ...
    def on_repeat_mode_changed(self, text):
        if text == "Custom":
            ...
            self.repeat_custom_days_spinbox.setEnabled(False)
            self.repeat_custom_hours_spinbox.setEnabled(False)
        # Show the "Only Final Reminder" option only if a repeating mode (other than None) is selected.
        self.only_final_checkbox.setVisible(text != "None")

    def on_only_final_changed(self, checked):
        # When the checkbox is toggled in the note detail view, update the note's attribute.
        if self.current_note_uid:
            note = next((n for n in self.notes if n.uid == self.current_note_uid), None)
            if note:
                note.suppress_dynamic_reminders = checked
                self.save_notes()
                self.update_notes_list()
    ...
    # ---------------------------
    # The remaining methods (tray, timer, CRUD, load/save, check_reminders, compute_next_reminder,
    # compute_next_occurrence, show_reminder, sticky logic) remain unchanged.
    def init_tray_icon(self):
        self.tray_icon = QSystemTrayIcon(self.style().standardIcon(QStyle.SP_ComputerIcon), self)
        self.tray_icon.setToolTip("RemindMeFy")
        tray_menu = QMenu()
    ...
    def load_note_details(self, item):
        ...
        # Show the "Only Final Reminder" checkbox if repeating note and occurrence is today.
        if note.repeat_mode != "None" and now.date() == note.next_occurrence.date():
            self.only_final_checkbox.setVisible(True)
            self.only_final_checkbox.setChecked(note.suppress_dynamic_reminders)
        else:
            self.only_final_checkbox.setVisible(False)
